...
const FileUpload = () => {
  ...
  const [plagiarismResult, setPlagiarismResult] = useState(null); // State for plagiarism results
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      // Assuming the backend returns a result with 'matches' and other details
      setPlagiarismResult(plagiarismResponse.data);
      toast.success("Plagiarism check completed!");

    } catch (error) {
      console.error(error);
      toast.error("Error uploading file or checking plagiarism.");
    } finally {
      setUploading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto py-12 px-4">
              ...
              {uploading ? (
                <>
                  <svg className="animate-spin mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 0 8-8v4a4 4 0 00-4 4H4z" />
                  </svg>
                  Uploading...
                </>
              ) : (
        ...
        {plagiarismResult && (
          <div className="bg-white p-4 rounded-md shadow-md mt-6">
            <h2 className="text-xl font-semibold mb-2">Plagiarism Check Results:</h2>
            <p><strong>Total Phrases Checked:</strong> {plagiarismResult.total_phrases}</p>
            <p><strong>Matched Count:</strong> {plagiarismResult.matched_count}</p>
